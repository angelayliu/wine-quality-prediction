...
class NeuralNetwork():
    ...
    def forward_pass(self, X) -> (List[np.ndarray], List[np.ndarray]):
        ...
        n = len(X)
        X = np.insert(X, 0, 1, axis=1) # X now size (n, f+1)
        for i in range(self.n_layers):
            a = np.empty((n, self.W[i].shape[1]))
            if (i == 0):
                a = np.dot(X, self.W[i]) # W is size (f+1, layer_sizes[i]) ?
            else:
                temp_z = np.insert(Z_vals[i-1], 0, 1, axis=1)
                a = np.dot(temp_z, self.W[i])
            activation = self.activations[i]
            z = activation.value(a)
            A_vals.append(a)
            Z_vals.append(z)
    ...
    def backward_pass(self, A_vals, dLdyhat) -> List[np.ndarray]:
        ...
        for i in range(layers - 1, -1, -1):
            delta = np.empty((len(A_vals[i]), self.W[i].shape[1]))
            activation = self.activations[i]
            a_prime = activation.derivative(A_vals[i])
            if (i == layers - 1):
                delta = dLdyhat * a_prime
            else:
                next_delta = deltas[0]
                temp_W = np.transpose(self.W[i+1][1:])
                delta = np.dot(next_delta, temp_W) * a_prime
    ...
    def update_weights(self, X, Z_vals, deltas) -> List[np.ndarray]:
        ...
        X = np.insert(X, 0, 1, axis=1) # X now size (n, f+1)
        for i in range(self.n_layers):
            row_of_zeros = np.zeros((1, self.W[i].shape[1]), dtype=self.W[i].dtype)
            if (i == 0):
                XT = np.transpose(X)
                dLdW = np.dot(XT, deltas[i])
                update = self.learning_rate * dLdW
                self.W[i] = self.W[i] - update
            else:
                zT = np.transpose(Z_vals[i-1])
                dLdW = np.dot(zT, deltas[i])
                update = self.learning_rate * dLdW
                self.W[i] = self.W[i] - np.vstack((row_of_zeros, update))
        return self.W
